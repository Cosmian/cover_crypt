import pickle
from cosmian_cover_crypt import Policy, PolicyAxis, CoverCrypt, SymmetricKey, Attribute
...
sym_key, enc_header = cc.encrypt_header(
    policy, "Secrecy::High && Country::France", pk, additional_data, authenticated_data
)

crypted_data = cc.encrypt_symmetric_block(sym_key, plaintext_bytes, authenticated_data)

decrypted_sym_key, metadata = cc.decrypt_header(
    sec_high_fr_sp_user, enc_header, authenticated_data
)

sym_key_bytes = decrypted_sym_key.to_bytes()
print("Key bytes", sym_key_bytes)

reconstructed_key = SymmetricKey.from_bytes(sym_key_bytes)

decrypted_data = cc.decrypt_symmetric_block(
    reconstructed_key, crypted_data, authenticated_data
)

assert str(bytes(decrypted_data), "utf-8") == plaintext

copy_policy = policy.clone()
policy.rotate(Attribute("Country", "France"))
print(policy, policy.to_string())
print(copy_policy, copy_policy.to_string())
