...
decrypted_data = cc.decrypt_symmetric_block(
    decrypted_sym_key, crypted_data, authenticated_data
)
...
msk, pk = cc.generate_master_keys(policy)
new_low_secret_mkg_data = cc.encrypt(
    policy,
    "Secrecy::Low && Country::France",
    pk,
    plaintext_bytes,
    additional_data,
    authenticated_data,
...
try:
    cleartext = cc.decrypt(
        sec_low_fr_sp_user, new_low_secret_mkg_data, authenticated_data
    )
except Exception as ex:
    print(f"As expected, user cannot decrypt this message: {ex}")
