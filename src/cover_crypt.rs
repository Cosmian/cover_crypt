...
pub type PrivateKey<A> =
    HashMap<A, <<X25519Crypto as AsymmetricCrypto>::KeyPair as KeyPair>::PrivateKey>;

/// CovCrypt public keys are a set of KEM public keys.
pub type PublicKey<A> =
    HashMap<A, <<X25519Crypto as AsymmetricCrypto>::KeyPair as KeyPair>::PublicKey>;
...
pub fn setup<A: Clone + Eq + Hash, R: CryptoRng + RngCore>(
    rng: &Mutex<R>,
    S: &HashSet<A>,
...
pub fn join<A: Clone + Debug + Eq + Hash>(
    msk: &PrivateKey<A>,
    U: &HashSet<A>,
) -> Result<PrivateKey<A>, Error<A>> {
...
/// - `T`   : target groups
/// - `S`   : user groups
pub fn encaps<A: Clone + Debug + Eq + Hash, R: CryptoRng + RngCore>(
    rng: &Mutex<R>,
    mpk: &PublicKey<A>,
    T: &HashSet<A>,
) -> Result<(SecretKey, Encapsulation<A>), Error<A>> {
    // secret key
    let K = utils::generate_random_bytes(rng, X25519Crypto::SECRET_KEY_LENGTH);
    ...
    for authorisation in T.iter() {
        match mpk.get(authorisation) {
            Some(pk) => {
                let (K_i, E_i) =
                    X25519Crypto::encaps(rng, pk).map_err(|err| Error::CryptoError { err })?;
                E.insert(
...
pub fn decaps<A: Debug + Eq + Hash>(
    sk_u: &PrivateKey<A>,
    E: &Encapsulation<A>,
) -> Result<Option<SecretKey>, Error<A>> {
    for (authorisation, (Ki_1, E_i)) in E.iter() {
        match sk_u.get(authorisation) {
            Some(sk) => {
                let Ki_2 =
                    X25519Crypto::decaps(sk, E_i).map_err(|err| Error::CryptoError { err })?;
