...
pub type PrivateKey<A, AC: Kem> =
    HashMap<A, <<AC as AsymmetricCrypto>::KeyPair as KeyPair>::PrivateKey>;

/// CovCrypt public keys are a set of KEM public keys.
pub type PublicKey<A, AC: Kem> =
    HashMap<A, <<AC as AsymmetricCrypto>::KeyPair as KeyPair>::PublicKey>;
...
/// secret key.
pub fn setup<A, R, AC>(rng: &Mutex<R>, S: &HashSet<A>) -> (PrivateKey<A, AC>, PublicKey<A, AC>)
where
...
pub fn join<A, AC>(msk: &PrivateKey<A, AC>, U: &HashSet<A>) -> Result<PrivateKey<A, AC>, Error<A>>
where
    A: Clone + Eq + Hash + Debug,
    AC: Kem,
{
...
{
    // secret key
    let K = utils::generate_random_bytes(rng, AC::SECRET_KEY_LENGTH);
    ...
    for authorisation in T.iter() {
        match mpk.get(authorisation) {
            Some(pk) => {
                let (K_i, E_i) = AC::encaps(rng, pk).map_err(|err| Error::CryptoError { err })?;
                E.insert(
...
pub fn decaps<A, R, AC>(
    sk_u: &PrivateKey<A, AC>,
    E: &Encapsulation<A>,
    ...
    AC: Kem,
{
    for (authorisation, (Ki_1, E_i)) in E.iter() {
        if let Some(sk) = sk_u.get(authorisation) {
            let Ki_2 = AC::decaps(sk, E_i).map_err(|err| Error::CryptoError { err })?;
...
mod tests {
    ...
    fn test_cover_crypt() -> Result<()> {
        ...
        let sk0 = join(&msk, &U[0])?;
        let sk1 = join(&msk, &U[1])?;
        // encapsulate for the target set
        let (K, E) = encaps(&rng, &mpk, &T)?;
        // decapsulate for users 1 and 3
        let res0 = decaps(&sk0, &E)?;
        let res1 = decaps(&sk1, &E)?;
